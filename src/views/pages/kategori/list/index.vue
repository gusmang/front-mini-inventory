    ...
    <div>
        ...
        <v-dialog v-model="dialogDel" max-width="390">
            ...
            <v-card>
                ...
                <v-card-text>Are you sure want to delete category ?</v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
                ...
                <v-expansion-panels v-model="panel" multiple>
                    <v-expansion-panel expand focusable>
                        <v-expansion-panel-content>
                            <v-layout>
                                <v-flex>
                                    <v-text-field v-model="$store.state.kategori.searchForm.name" label="Search By Name"
                                        single-line small clearable @keyup.13="fetch(1)">
                                    </v-text-field>
                                </v-flex>
                            </v-layout>
                            <v-layout>
            ...
            <template v-slot:datatable>
                <v-flex md12 style="overflow-x: scroll">
                    <v-data-table :headers="headers" :items="listKategori" :options.sync="pagination"
                        :server-items-length="$store.state.kategori.attributes.total" :footer-props="{
                                ...
                                <template v-slot:default="dialog">
                                    <v-card>
                                        ...
                                        <v-container>
                                            <v-layout>
                                                <v-flex>
                                                    <v-text-field v-model="form.editNama" label="Nama Kategori">
                                                    </v-text-field>
                                                </v-flex>
                                            </v-layout>
...
import ModalForm from "../dialog/addKategori.vue";
...
export default {
    ...
    methods: {
        ...
        findKategori(item) {
            ...
            this.form.editNama = item.nama_kategori
        },
        showAdd() {
        ...
        edit(dlg) {
            ...
            this.$store
                .dispatch("kategori/update", {
                    id: this.form.userId,
                    nama_kategori: this.form.editNama
                })
                .then(() => {
                    this.$store.commit("main/setLoading", false);
                    this.loading = false;
    ...
    computed: {
        listKategori() {
            return this.$store.state.kategori.attributes.list.map(
                (items, index) => ({
                    ...items,
                    no: index + 1,
